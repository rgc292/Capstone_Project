...
for line in sys.stdin:
    ...
    for l in csv_reader:
        ...
        lat_lon = re.sub('[()]', '', l[-1])
        lat = lat_lon.split(',')[0].strip()
        lon = lat_lon.split(',')[-1].strip()
        
        try:

			if (lon == 'Location 1' or lat == '' or lon == ''):
				pass
				...
				try:
					...
					if not(crime):
						...
						month = date.split('/')[0]
						day = date.split('/')[1]
						#print '%s' %(l[2].split(' ')[1].split(':')[0])
						time = float(l[2].split(' ')[1].split(':')[0]) + float(l[2].split(' ')[1].split(':')[1])/float(60) + float(l[2].split(' ')[1].split(':')[2])/float(3600)

						if(int(year) == 2011 and l[11].split(' ')[0].upper() == 'GRAND'): # == 2012 ...
							print "%s,%0.10f,%s,%s" %(year+'-'+month+'-'+day,time,lat,lon)
