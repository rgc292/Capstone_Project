...
for line in sys.stdin:
    ...
    for l in csv_reader:
		...
		try:
			if (l[0] == 'vendor_id' or l[0] == '' or float(l[4]) > .101 or float(l[4]) < .01):
					pass

			else:
				try:
						pick = simplePolygonTest_lga(float(l[6]), float(l[5]))
						drop = simplePolygonTest_lga(float(l[10]), float(l[9]))
						...
						if not(pick and drop):
								...
								speed = float(l[4])/float(duration)
								if speed > 25:
								    print '%s,%0.10f,%0.10f,%0.10f,%0.10f,%s,%s,%s,%s' %(date_p[:10],time_p,time_d,duration,speed,l[6],l[5],l[10],l[9])
				except:
					pass
