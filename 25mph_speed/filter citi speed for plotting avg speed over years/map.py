...
for line in sys.stdin:
    ...
    for l in csv_reader:
		...
		try:
			if (l[0] == 'tripduration' or l[1] == '' or l[2] == '' or l[5] == '' or l[6] == '' or l[9] == '' or  l[10] == ''):
				pass
				...
				lon2 = float(l[10])
				try:
					pick = simplePolygonTest_lga(lat1, lon1)
					drop = simplePolygonTest_lga(lat2, lon2)
					...
					if not(pick and drop):
						...
						if len(date_p[0]) == 4:
						    date_p = date_p[0] + '-' + date_p[1]
						else:
						    date_p = date_p[2] + '-' + date_p[0]
						
						duration = l[0]
						duration = float(duration)/3600.0
						lat_d = dist(lat1,lat2,'lat')
						lon_d = dist(lon1,lon2,'lon')
						tot_d = lat_d + lon_d
						
						if duration < 0.000:
						    duration = 0.001
						    
						speed = tot_d/duration
						
						if (speed <=0.0 or speed > 40.0):
						    pass
						else:
						    print "%s\t%d,%0.10f" %(date_p,1,speed)
