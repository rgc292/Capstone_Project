...
def simplePolygonTest_lga(lat,lon):
    ...
    # Test if the polygon contains the point
...
for line in sys.stdin:
    ...
    for l in csv_reader:
        ...
        try:
				if ((l[0].upper() == 'DATE' or lat == '' or lon == '' or lat == '0' or lon == '0') or (int(l[10]) == 0 and int(l[11]) == 0)):
						pass

				else:
						try:
						
								crime = simplePolygonTest_lga(float(lat), float(lon))
								...
								if not(crime):
										...
										injured = int(l[10])
										killed =  int(l[11])
										pedestrian_injured = int(l[12])
										pedestrian_killed = int(l[13])
										cyclist_injured = int(l[14]) 
										cyclist_killed = int(l[15])
										motorist_injured = int(l[16])
										motorist_killed = int(l[16])
					
										if(int(year) >= 2011 and int(year) <= 2016):
											print '%s,%d,%d,%d,%d,%d,%d,%d,%d,%s,%s' %(year+'-'+month+'-'+day,injured,killed,pedestrian_injured,pedestrian_killed,cyclist_injured,cyclist_killed,motorist_injured,motorist_killed,lat,lon)
